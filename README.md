...
При каждом запуске Launcher автоматически проверяет обновление самого приложения. Если в репозитории лаунчера доступна новая версия, игрок получает понятное предложение обновить её. После подтверждения `Veles.Updater.exe` ждёт закрытия Launcher, скачивает `VelesLauncherSetup.exe`, запускает новый установщик и завершает старый процесс. Отдельного updater setup нет, ручные команды не нужны.
