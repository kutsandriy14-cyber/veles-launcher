...
Решение `VelesLauncher.sln` содержит четыре проекта. `Veles.Core` — общая библиотека без интерфейса. `Veles.Launcher` — обычный пользовательский лаунчер. `Veles.BuildPublisher` — полностью отдельная админ-панель, которая не входит в клиентский лаунчер и хранит GitHub-токен только в памяти. `Veles.Updater` — отдельная программа, проверяющая релизы самого лаунчера.
...
Сборка выполняется в Visual Studio на Windows с workload «.NET desktop development». Откройте `VelesLauncher.sln`, выберите `Release` и соберите нужный проект отдельно. На текущем этапе setup-установщики сознательно не включены: они будут добавлены после проверки основных приложений и сценариев.

Для автоматической сборки Windows-артефактов предусмотрен `.github/workflows/windows-build.yml`. После публикации релиза лаунчера его asset должен называться `VelesLauncherSetup.exe`, чтобы отдельный Updater мог его найти.
