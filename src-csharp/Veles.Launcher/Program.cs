...
namespace Veles.Launcher
{
    ...
    internal sealed class LauncherForm : Form
    {
        ...
        private void BuildUi()
        {
            ...
            var actions = new FlowLayoutPanel { Dock = DockStyle.Fill, FlowDirection = FlowDirection.RightToLeft, WrapContents = true, Padding = new Padding(0, 5, 0, 0), BackColor = Background }; var site = Button("Сайт сервера", Color.FromArgb(33, 25, 19), TextColor); ApplyIcon(site, "server.png"); site.Click += (s, e) => OpenUrl("https://veles-saite.vercel.app/"); var checkLauncher = Button("Обновить Launcher", Color.FromArgb(33, 25, 19), TextColor); ApplyIcon(checkLauncher, "update.png"); checkLauncher.Click += async (s, e) => await CheckLauncherUpdateAsync(true); _settingsButton = Button("Настройки", Color.FromArgb(33, 25, 19), TextColor); ApplyIcon(_settingsButton, "settings.png"); _settingsButton.Click += (s, e) => ShowSettings(); actions.Controls.Add(site); actions.Controls.Add(checkLauncher); actions.Controls.Add(_settingsButton); header.Controls.Add(_brand, 0, 0); header.Controls.Add(actions, 1, 0); root.Controls.Add(header, 0, 0); root.SetColumnSpan(header, 2);
        ...
        {
            var panel = CardPanel(); var layout = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 1, RowCount = 5, Padding = new Padding(20), BackColor = Card, Margin = new Padding(0) }; layout.RowStyles.Add(new RowStyle(SizeType.Absolute, 34)); layout.RowStyles.Add(new RowStyle(SizeType.Absolute, 22)); layout.RowStyles.Add(new RowStyle(SizeType.Absolute, 38)); layout.RowStyles.Add(new RowStyle(SizeType.Absolute, 42)); layout.RowStyles.Add(new RowStyle(SizeType.Percent, 100)); panel.Controls.Add(layout); layout.Controls.Add(SectionHeader("server.png", "ПОДКЛЮЧЕНИЕ К СЕРВЕРУ"), 0, 0); layout.Controls.Add(Label("IP АДРЕС СЕРВЕРА", 0, 0, 9, Muted, true), 0, 1); _address = new Label { Text = "—", Dock = DockStyle.Fill, Font = new Font("Segoe UI", 18, FontStyle.Bold), ForeColor = TextColor, AutoEllipsis = true, TextAlign = ContentAlignment.MiddleLeft }; layout.Controls.Add(_address, 0, 2); var copyRow = new FlowLayoutPanel { Dock = DockStyle.Fill, FlowDirection = FlowDirection.LeftToRight, WrapContents = false, BackColor = Card, Margin = new Padding(0) }; var copy = Button("Копировать", Color.FromArgb(43, 28, 20), TextColor); ApplyIcon(copy, "server.png"); copy.Width = 126; copy.Click += (s, e) => { if (_address.Text != "—") { Clipboard.SetText(_address.Text); ShowNotice("IP сервера скопирован.", false); } }; copyRow.Controls.Add(copy); layout.Controls.Add(copyRow, 0, 3); layout.Controls.Add(new Label { Text = "Адрес читается из build-info.txt текущего релиза.", Dock = DockStyle.Fill, Font = new Font("Segoe UI", 9), ForeColor = Muted, AutoEllipsis = true, TextAlign = ContentAlignment.TopLeft }, 0, 4); return panel;
        }

        private Control MakeBuildCard()
        {
            var panel = CardPanel(); var layout = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 1, RowCount = 5, Padding = new Padding(20), BackColor = Card, Margin = new Padding(0) }; layout.RowStyles.Add(new RowStyle(SizeType.Absolute, 34)); layout.RowStyles.Add(new RowStyle(SizeType.Absolute, 30)); layout.RowStyles.Add(new RowStyle(SizeType.Absolute, 28)); layout.RowStyles.Add(new RowStyle(SizeType.Absolute, 46)); layout.RowStyles.Add(new RowStyle(SizeType.Percent, 100)); panel.Controls.Add(layout); layout.Controls.Add(SectionHeader("build.png", "СБОРКА И ОБНОВЛЕНИЕ"), 0, 0); _profile = new Label { Text = "Сборка не загружена", Dock = DockStyle.Fill, Font = new Font("Segoe UI", 13, FontStyle.Bold), ForeColor = TextColor, AutoEllipsis = true, TextAlign = ContentAlignment.MiddleLeft }; layout.Controls.Add(_profile, 0, 1); _meta = new Label { Text = "Сборка сервера пока не опубликована.", Dock = DockStyle.Fill, Font = new Font("Segoe UI", 9), ForeColor = Muted, AutoEllipsis = true, TextAlign = ContentAlignment.MiddleLeft }; layout.Controls.Add(_meta, 0, 2); var buttons = new FlowLayoutPanel { Dock = DockStyle.Fill, FlowDirection = FlowDirection.LeftToRight, WrapContents = true, BackColor = Card, Margin = new Padding(0) }; _update = Button("Проверить и обновить сборку", Orange, Color.FromArgb(23, 13, 6)); ApplyIcon(_update, "update.png"); _update.Width = 235; _update.Click += async (s, e) => await InstallAsync(); buttons.Controls.Add(_update); _launch = Button("Запустить Minecraft", Color.FromArgb(43, 28, 20), TextColor); ApplyIcon(_launch, "play.png"); _launch.Width = 190; _launch.Enabled = false; _launch.Click += async (s, e) => await LaunchAsync(); buttons.Controls.Add(_launch); layout.Controls.Add(buttons, 0, 3); _progress = new ProgressBar { Dock = DockStyle.Bottom, Height = 6, Style = ProgressBarStyle.Continuous, Maximum = 100, Visible = false }; layout.Controls.Add(_progress, 0, 4); return panel;
        }
