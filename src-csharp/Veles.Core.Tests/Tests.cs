...
namespace Veles.Core.Tests
{
    internal static class Tests
    {
        ...
        {
            var config = "\uFEFF# comment\nBUILD_NAME=Test\nBUILD_VERSION=0.12.7\nMINECRAFT_VERSION=1.20.1\nMOD_LOADER=Forge\nMOD_LOADER_VERSION=47.2.0\nSERVER_ADDRESS=213.152.43.53:25589\nJAVA_VERSION=17\nJAVA_VENDOR=BellSoft Liberica\nJAVA_RUNTIME_PATH=runtime\\java\nMOD_LOADER_PROFILE=launch.json\n";
            var info = BuildInfo.Parse(config); info.Validate();
            Assert(info.BuildVersion == "0.12.7", "Build version parser failed"); Assert(info.ServerAddress == "213.152.43.53:25589", "Server address parser failed"); Assert(info.JavaVersion == "17", "Java version parser failed"); Assert(info.JavaVendor == "BellSoft Liberica", "Java vendor parser failed");
