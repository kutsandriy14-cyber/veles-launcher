...
namespace Veles.Core.Tests
{
    internal static class Tests
    {
        ...
        {
            var config = "\uFEFF# comment\nBUILD_NAME=Test\nBUILD_VERSION=0.12.7\nMINECRAFT_VERSION=1.20.1\nMOD_LOADER=Forge\nMOD_LOADER_VERSION=47.2.0\nSERVER_IP=213.152.43.53\nSERVER_PORT=25589\n";
            var info = BuildInfo.Parse(config); info.Validate();
            Assert(info.BuildVersion == "0.12.7", "Build version parser failed"); Assert(info.ServerAddress == "213.152.43.53:25589", "Server address parser failed");
            ...
            var invalid = BuildInfo.Parse(config.Replace("SERVER_PORT=25589", "SERVER_PORT=70000")); var rejected = false; try { invalid.Validate(); } catch (InvalidDataException) { rejected = true; } Assert(rejected, "Invalid port was not rejected");
