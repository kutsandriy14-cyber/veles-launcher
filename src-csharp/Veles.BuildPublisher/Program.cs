...
namespace Veles.BuildPublisher
{
    ...
    internal sealed class ReleaseManagerForm : Form
    {
        ...
        private async Task SetSelectedAsync(bool active, int priority) { var selected = Selected(); if (selected == null) return; try { if (active) foreach (var release in _releases) if (!string.Equals(release.Id, selected.Id, StringComparison.OrdinalIgnoreCase)) await _api.UpdateReleaseAsync(release.Id, release.Name, Markers(release.Body, false, 9999), CancellationToken.None); await _api.UpdateReleaseAsync(selected.Id, selected.Name, Markers(selected.Body, active, priority), CancellationToken.None); await LoadAsync(); } catch (Exception error) { _status.Text = "Ошибка изменения статуса: " + error.Message; } }
